...
public class Job {
    ...
    //      ตอบ ใช้ logger.now() จาก ProjectLogger ตัวเดียวที่สร้างใน Main
    //   2. ฟิลด์ใดถูกเขียนโดย Thread หนึ่งแล้วอ่านโดยอีก Thread หนึ่ง
    //      และต้องป้องกันอย่างไร
    //      ตอบ ฟิลด์ actualArrivalTime, startTime, completionTime, resourceWaitStartTime, resourceWaitMs
    //      ต้องใช้คำสั่ง synchronized หรือใช้ฟิลด์ volatile เพื่อให้ Thread อื่นมองเห็นค่าล่าสุด
    //   3. ผลที่ได้ต้องสอดคล้องกับสมการตรวจสอบในหัวข้อ 8:
    //      Turnaround = Waiting + workMs + Resource Wait + resourceMs
    // =====================================================================

    /** เวลาที่ Job เข้าระบบจริง จาก logger.now() [volatile เพื่อให้ Thread อื่นมองเห็นค่าล่าสุด]*/
    public volatile long actualArrivalTime = -1;

    /** เวลาที่ Worker เริ่มทำ Job จาก logger.now() */
    public volatile long startTime = -1;

    /** เวลาที่ Job ทำเสร็จทั้งหมด จาก logger.now() */
    public volatile long completionTime = -1;

    /** เวลาที่เริ่มรอ Semaphore */
    public volatile long resourceWaitStartTime = -1;

    /** เวลารอ Semaphore รวม */
    public volatile long resourceWaitMs = 0;

    /** ตั้งแต่ Job เข้ามาจริง จนถึง Worker เริ่มทำ Job ใช้เวลารอกี่ ms */
    public long waitingTime() {
        return startTime - actualArrivalTime;
    }
    /** ตั้งแต่ Job เข้ามา จนกระทั่ง Job เสร็จทั้งหมด ใช้เวลารวมเท่าไร */
